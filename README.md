...
Обход ограничений платформы HH.Ru при прохождении тестов на подтверждение IT-компетенций и других тестов.
...
![Минцифры](./images/mincifri.png "Минцифры")
...
Ролик на YouTube: https://www.youtube.com/watch?v=N5uQxNy76Mk
Ролик на RuTube: https://rutube.ru/video/da035a7b988ae96653afccb538fb3631/
...
Оператором платформы является HH.Ru. Считается, что эти тесты отражают требования рынка, разработаны работодателями совместно с образовательными партнёрами и размещены на HH.Ru. Спорный момент, но будем считать, что так оно и есть.

Более подробная информация и старт тестирования доступны по ссылке https://www.gosuslugi.ru/itskills
...
## Как долго будет работать данный хак?
...
Если Вы заметите, что расширение перестало работать или работает "как-то не так", то прошу Вас написать об этом мне в Telegram на @DTorretto и максимально подробно описать проблему, желательно с приложением скриншотов и/или видеозаписи. В свою очередь мы постараемся оперативно решить эту проблему. А если разработчики платформы тестирования придумают другие способы защиты, то мы постараемся найти оптимальное решение и обойти их.


## Расширение предназначено только для тестов на HH.Ru?
...
2. Поместите все файлы из скачанного архива в папку, созданную на предыдущем шаге.
...
2. Поместите скачанный ZIP-архив с расширением в папку, созданную на предыдущем шаге.
...
5. В открывшемся окне выберите Вашу ZIP-архив с расширением, который Вы положили в папку, созданную на первом шаге.
...
Чтобы предотвратить обнаружение смены вкладки (окна) или сворачивания браузера, нужно переопределять ключевые JavaScript-события и свойства. И вот как это делается:

- Переопределяем Page Visibility API (полная маскировка):
	- Заставляем document.hidden всегда возвращать false
	- Теперь document.visibilityState всегда возвращает 'visible'
	- Поддержка стандартных и префиксных версий (moz, webkit, ms)
	- Блокировка всех известных способов определения видимости	
...
- Делаем блокировку событий:
	...
	- Модифицирует addEventListener чтобы игнорировать подписки на эти события

- Защита от интервальных проверок:
	- Анализирует функции, передаваемые в setInterval
	- Блокирует частые проверки (интервалы ≤ 2 секунд) и проверки, содержащие ключевые слова
...
- Безопасность
	- Расширение использует только необходимые разрешения и работает в sandbox.

- ***В версии 1.1 (актуальная версия) реализованы следующие улучшения:***
...
## Как купить расширение Career Hacker
...
Задонатить автору можно на официальной странице расширения: https://careerhack.hitsecurity.ru/


## FAQ: Какие гарантии, что это не обман, не вирус и расширение действительно работает?
...
## FAQ: Что делать, если расширение перестанет работать после обновления платформы тестирования?
...
## FAQ: Можно ли использовать это расширение для других платформ (тестов), кроме HH.Ru?
...
## FAQ: Мне помог Career Hacker, и я хочу отблагодарить автора материально. Как это сделать?
...
Кнопка для донатов вверху страницы.

----
© 2025 Gosuslugi and HH.RU Career Hacker: Advanced Tab Tracking Blocker Browser Extension. Все права защищены.
© 2025 Andrey Zinoviev aka @DTorretto (Telegram) aka @Zevs_AV (Telegram), E-Mail: avzevs@gmail.com. Все права защищены.